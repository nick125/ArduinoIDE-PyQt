...
########################################################################
## Projects Browser - sketches and examples in 2 cols in central widget
########################################################################
class ProjectsBrowser(QtGui.QSplitter):
	def __init__(self,  parent, main):
		QtGui.QDockWidget.__init__(self, parent)

		self.main = main

		### TODO We could end up with n numer of therese, different file roots
		myProjectWidgets = ProjectListWidget("My Projects", self, self.main)
		self.addWidget(myProjectWidgets)
		self.connect(myProjectWidgets, QtCore.SIGNAL("open_project"), self.on_open_project)

		examplesWidget = ProjectListWidget("Example Projects", self, self.main)
		self.addWidget(examplesWidget)
		self.connect(examplesWidget, QtCore.SIGNAL("open_project"), self.on_open_project)
		
	def on_open_project(self, file_path):
		self.emit(QtCore.SIGNAL("open_project"), file_path)


#################################################################
## Sketches Tree/widgets - browses a directory - "ki" is set on title = "Examples" == dodgy
#################################################################
class ProjectListWidget(QtGui.QWidget):
	...
	def __init__(self, title, parent, main):
		...
		headLabel = GenericWidgets.HeaderLabel(self, self.main, icon=Ico.Projects, title=title, color="black", wash_to="#456D91")
	...
	def on_tree_double_clicked(self, item, col):
		...
		if not data.isNull():
			self.emit(QtCore.SIGNAL("open_project"), data.toString())
